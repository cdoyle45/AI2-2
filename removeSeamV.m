function imOut = removeSeamV(im4, seam)
    ...
    % Output: im4 with the seam removed
    ...
    for i = 1:1:size(seam, 1)
        ...
        imOut_R(i, :) = im_R(i, [1:seam(i)-1, seam(i)+1:end]);
        imOut_G(i, :) = im_G(i, [1:seam(i)-1, seam(i)+1:end]);
        imOut_B(i, :) = im_B(i, [1:seam(i)-1, seam(i)+1:end]);
        imOut_M(i, :) = im_M(i, [1:seam(i)-1, seam(i)+1:end]);
    end
